...
namespace ngl
{
    namespace math
    {
        ...
        */
        ...
        {
            // const Vec3 abs_v = Vec3::Abs(v);
            const Vec3 abs_v  = Vec3(local::abs(v.x), local::abs(v.y), local::abs(v.z));
            const int x_order = int(abs_v.x > abs_v.y) + int(abs_v.x > abs_v.z);
            const int y_order = int(abs_v.y >= abs_v.x) + int(abs_v.y > abs_v.z);
            const int z_order = int(abs_v.z >= abs_v.x) + int(abs_v.z >= abs_v.y);

            return Vec3i(x_order, y_order, z_order);
        }
        // ベクトルの各成分を昇順で並べ直すためのインデックスベクトルを返す.
        // 例) 入力Vec3( -5.0f, 2.0f, 3.0f )ならば、絶対値の大きさ順は (2, 0, 1) なので、戻り値は Vec3i(1,2,0) となる.
        static constexpr Vec3i GetVec3ComponentReorderIndexByMagnitude(const Vec3& v)
        {
            const Vec3i order = GetVec3ComponentOrderByMagnitude(v);
            Vec3i index_vec;
            index_vec.Component(order.x) = 0;
            index_vec.Component(order.y) = 1;
            index_vec.Component(order.z) = 2;
            return index_vec;
        }
        // GetVec3ComponentOrderByMagnitude のテストコード
        // 非ゼロのVec3についてGetVec3ComponentOrderByMagnitudeを実行して結果を検証.
        // 入力ベクトルの要素の絶対値の大きさの昇順の順序と、結果のインデックスベクトルが一致すること.
        // 同じ絶対値の要素がある場合はXYZの順序を維持する.
        // 最初にテスト用のベクトルを配列で列挙しておき、ループで順次検証する.
        inline void TestGetVec3ComponentOrderByMagnitude()
        {

            constexpr auto test00 = GetVec3ComponentOrderByMagnitude(Vec3(1.0f, 2.0f, 3.0f));
            constexpr auto test01 = GetVec3ComponentOrderByMagnitude(Vec3(1.0f, 3.0f, 3.0f));
            constexpr auto test02 = GetVec3ComponentOrderByMagnitude(Vec3(1.0f, 1.0f, 1.0f));



            constexpr Vec3 test_vectors[] = {
                Vec3(1.0f, 2.0f, 3.0f),
                Vec3(-1.0f, 2.0f, -3.0f),
                Vec3(3.0f, -2.0f, 1.0f),
                Vec3(0.5f, -1.5f, 1.0f),
            ...
            {
                const Vec3 abs_vec(local::abs(vec.x), local::abs(vec.y), local::abs(vec.z));
                const Vec3i order = GetVec3ComponentOrderByMagnitude(vec);
                const Vec3i reorder_index_vec = GetVec3ComponentReorderIndexByMagnitude(vec);

                // orderの要素が重複のない0,1,2であることをチェック.
                assert((order.x != order.y) && (order.y != order.z) && (order.z != order.x));
                assert((order.x >= 0) && (order.x <= 2));
                assert((order.y >= 0) && (order.y <= 2));
                assert((order.z >= 0) && (order.z <= 2));

                // orderの順序通りに絶対値が昇順であることをチェック.
                int check_indices[3] = { 0, 0, 0 };
                ...
                assert(order.x == check_indices[0]);
                assert(order.y == check_indices[1]);
                assert(order.z == check_indices[2]);

                // reorder_index_vec の順で昇順になっていることを要素を実際に比較チェック.
                assert(abs_vec.Component(reorder_index_vec.x) <= abs_vec.Component(reorder_index_vec.y));
                assert(abs_vec.Component(reorder_index_vec.y) <= abs_vec.Component(reorder_index_vec.z));
        ...
        inline void math_test()
        {
            ...
            TestGetVec3ComponentOrderByMagnitude();
        }
    }  // namespace math
}  // namespace ngl