...
namespace ngl::render
{
	...
	static constexpr char k_shader_test_path_prefx[] = "../ngl/shader/";
#define NGL_SHADER_TEST_PATH(shader_file) text::FixedString<128>("%s/%s", k_shader_test_path_prefx, shader_file)
...
;
				...
				rtg::RtgResourceHandle h_depth, rtg::RtgResourceHandle h_tex_compute, const SetupDesc& desc)
			{
...

				{
					...
					auto res_shader = ResourceMan.LoadResource<ngl::gfx::ResShader>(p_device, NGL_SHADER_TEST_PATH("screen/generate_lineardepth_cs.hlsl"), &loaddesc);
		...
		struct TaskLightPass : public rtg::IGraphicsTaskNode
		{
				...
				const SetupDesc& desc)
			{
				...
				
				{
					...
					auto res_shader_vs = ResourceMan.LoadResource<ngl::gfx::ResShader>(p_device, NGL_SHADER_TEST_PATH("screen/fullscr_procedural_vs.hlsl"), &loaddesc_vs);
					...
					auto res_shader_ps = ResourceMan.LoadResource<ngl::gfx::ResShader>(p_device, NGL_SHADER_TEST_PATH("df_light_pass_ps.hlsl"), &loaddesc_ps);
		...
		struct TaskFinalPass : public rtg::IGraphicsTaskNode
		{
				...
				const SetupDesc& desc)
			{
...

				{
					...
					auto res_shader_ps = ResourceMan.LoadResource<ngl::gfx::ResShader>(p_device, NGL_SHADER_TEST_PATH("final_screen_pass_ps.hlsl"), &loaddesc_ps);
					...
					pso_ = ngl::rhi::RhiRef<ngl::rhi::ComputePipelineStateDep>(new ngl::rhi::ComputePipelineStateDep());
					{
						ngl::rhi::ComputePipelineStateDep::Desc cpso_desc = {};
						{
							...
							auto cs_load_handle = ngl::res::ResourceManager::Instance().LoadResource<ngl::gfx::ResShader>(
								p_device, NGL_SHADER_TEST_PATH("test/async_task_test_cs.hlsl"), &cs_load_desc
							);
				...
				{
					{
						...
						res_shader_lib_ = ResourceMan.LoadResource<ngl::gfx::ResShader>(p_device, NGL_SHADER_TEST_PATH("dxr_sample_lib.hlsl"), &loaddesc);
					}
