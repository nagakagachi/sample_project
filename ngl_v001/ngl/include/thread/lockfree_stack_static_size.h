...
namespace thread {
    ...
    /**
     * @brief コンパイル時にサイズが静的に決定されるロックフリースタック実装
     * 
     ...
     * 
     * 特徴：
     * - コンパイル時にサイズが決定（テンプレートパラメータ）
     * - スタックメモリを使用（std::array）
     * - キャッシュ効率が高い
     * - メモリレイアウトが静的
     * 
     * 制限事項：
     ...
     * @tparam MaxSize スタックの最大要素数（コンパイル時定数）
     */
    ...
    public:
        StaticSizeLockFreeStack() : head_(INVALID_INDEX) {
            // フリーリストの初期化
