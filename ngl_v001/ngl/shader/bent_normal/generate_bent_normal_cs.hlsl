...
BentNormal生成テスト, ついでにAOも計算してw成分に格納.
...
{
    const float view_z_right = TexLinearDepth.Load(int3(texel_pos + int2(1, 0), 0)).r;
    const float view_z_down = TexLinearDepth.Load(int3(texel_pos + int2(0, 1), 0)).r;
