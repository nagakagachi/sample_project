...
{
    const uint output_lod = (uint)cb_srvs.sap_build_lod;
    const uint2 out_size = uint2(SapLodWidthFromCb(output_lod), SapLodHeightFromCb(output_lod));
    ...
    for(int child_index = 0; child_index < 4; ++child_index)
    {
        ...
        child_node[child_index] = SapLoadNode(output_lod - 1u, dtid.xy * 2u + child_offset);
    ...
    {
        SapStoreNode(output_lod, dtid.xy, SapMakeInvalidNode());
        return;
    }
    ...
    if(0 == solid_child_count)
    {
        ...
        SapStoreNode(output_lod, dtid.xy, SapMakeEmptyNode());
        return;
    }
    ...
    SapStoreNode(output_lod, dtid.xy, out_node);
}
