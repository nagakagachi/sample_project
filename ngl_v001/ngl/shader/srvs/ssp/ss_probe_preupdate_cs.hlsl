...
#define SEARCH_GROUP_SIZE_X 3
#define SEARCH_GROUP_SIZE_Y 3
...
#define SEARCH_THREAD_INDEX_BITS 4u
#define SEARCH_THREAD_INDEX_MASK ((1u << SEARCH_THREAD_INDEX_BITS) - 1u)

#if SEARCH_THREAD_COUNT > (1 << SEARCH_THREAD_INDEX_BITS)
    #error "Octa SSP preupdate search group size exceeds SEARCH_THREAD_INDEX_BITS capacity."
#endif
...
)
{
    ...
    ScreenSpaceProbeHistoryTileInfoTex.GetDimensions(tile_info_size.x, tile_info_size.y);
    if(any(gid.xy >= tile_info_size))
        return;

    const int2 probe_id = gid.xy;// フル解像度に対して 1/SCREEN_SPACE_PROBE_INFO_DOWNSCALE で, ScreenSpaceProbeごとに1テクセル.
    ...
    if(0 == gindex)
    {
        ...
        if(isValidDepth(probe_depth))
        {
            ...
            {
                // ごく稀に深度からの復元が外れ値になるため, 近傍Probe法線平均との差分で置換する.
                ...
                for(int i = 0; i < 4; ++i)
                {
                    ...
                    const float4 neighbor_info = ScreenSpaceProbeHistoryTileInfoTex[neighbor_tile_id];
    ...
    if(0 != gs_probe_valid)
    {
        ...
        RWScreenSpaceProbeTileInfoTex[probe_id] = SspTileInfoBuild(gs_probe_depth, gs_probe_pos_in_tile, gs_probe_normal_oct, is_reprojection_succeeded);
        RWScreenSpaceProbeBestPrevTileTex[probe_id] = gs_best_prev_tile_packed;
    }
    else
    {
        ...
        RWScreenSpaceProbeBestPrevTileTex[probe_id] = 0xffffffffu;
    }
}
