...
)
{
    ...
    const uint elem_count = cb_srvs.wcp.grid_resolution.x * cb_srvs.wcp.grid_resolution.y * cb_srvs.wcp.grid_resolution.z;
    ...
    // Cell中心.
    const float3 probe_cell_center = (float3(voxel_coord) + 0.5) * cb_srvs.wcp.cell_size + cb_srvs.wcp.grid_min_pos;
    ...
    // Probeレイサンプル.
    {
    ...
    #endif
        {
            // Probe更新.
                ...
                const float random_angle = noise_float_to_float(float2(voxel_index, cb_srvs.frame_count)) * 6.28318530718;// 0~2π
                ...
                float3 sample_ray_dir = random_unit_vector3( float2( cb_srvs.frame_count + sample_index, update_element_id + sample_index * 37 ) );
            #endif
            ...
            // レイキャスト.
#if NGL_SRVS_TRACE_USE_HIBRICK_WCP_ELEMENT_UPDATE
