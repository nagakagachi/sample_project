...
namespace ngl
{
	namespace rhi
	{
		...
		EnhancedBarrierStateInfo ConvertResourceStateToEnhancedBarrierInfo(EResourceState v)
		{
			...
			{
			case EResourceState::Common:
				return { D3D12_BARRIER_SYNC_ALL, D3D12_BARRIER_ACCESS_COMMON, D3D12_BARRIER_LAYOUT_COMMON };
			...
			case EResourceState::ShaderRead:
				// PIXEL_SHADER_RESOURCE の等価だが ALL_SHADING で安全側に寄せる.
			...
			case EResourceState::DepthRead:
				return { D3D12_BARRIER_SYNC_DEPTH_STENCIL, D3D12_BARRIER_ACCESS_DEPTH_STENCIL_READ, D3D12_BARRIER_LAYOUT_DEPTH_STENCIL_READ };

			case EResourceState::IndirectArgument:
				return { D3D12_BARRIER_SYNC_EXECUTE_INDIRECT, D3D12_BARRIER_ACCESS_INDIRECT_ARGUMENT, D3D12_BARRIER_LAYOUT_COMMON };
